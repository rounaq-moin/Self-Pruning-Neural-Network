...
class PrunableLinear(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        # MODIFIED INIT (not constant 5.0)
...
class SelfPruningNet(nn.Module):
    ...
    def sparsity_loss(self):
        ...
        return torch.cat(gates).mean()   # keep MEAN (your friend used this)

    def overall_sparsity(self, threshold=0.05):  # MODIFIED threshold
        total, pruned = 0, 0
        for layer in self.prunable_layers():
            g = layer.get_gates()
...
def get_loaders(batch=128):
    ...
    train_tf = transforms.Compose([
        ...
        transforms.ColorJitter(brightness=0.2, contrast=0.2),  # MODIFIED
        transforms.ToTensor(),
...
def run(lam, train_loader, test_loader, device):
    ...
    scheduler = optim.lr_scheduler.StepLR(opt, step_size=10, gamma=0.5)  # MODIFIED
